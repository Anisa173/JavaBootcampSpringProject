...
@Modifying
	...
	@Query(" Select t.tableName ,t.tableDescription , t.noChairsT From RestorantTables t INNER JOIN Restorant r ON t.r.restorant_tbId = t.rtb_id Where t.rtb_id =: rtb_id"
			+ " And r.idRestorant IN (Select r.restName From Restorant r INNER JOIN User a ON a.r.adminRId = a.id Where r.idRestorant =: ?1 And a.id =: ?2)")
	RestorantTables getAlltablesById(Integer rtb_id, Integer adminRId);
