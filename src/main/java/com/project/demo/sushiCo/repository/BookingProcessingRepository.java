...
public interface BookingProcessingRepository extends JpaRepository<BookingProcessing, Integer> {
	...
	@Query("Select c.id , cr.cR_Id , r.idRestorant "
			...
			+ " Where c.id =: ?1 and cr.cR_Id =: ?2  and adw.id  IN "
			+ "( Select r.idRestorant From User adw Inner Join Restorant r ON adw.id = adw.r.adminIdWeb  Where r.idRestorant =: idRestorant)")
...
/*IN ( Select r.idRestorant =: ?1  From rtb Inner Join Restorant r ON rtb.rtb_id = rtb.r.idRestorant Where rtb.rtb_id =: ?1 and rtb.r.idRestorant =: ?2  )*/
