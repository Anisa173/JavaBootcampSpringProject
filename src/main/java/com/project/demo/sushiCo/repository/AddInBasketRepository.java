...
public interface AddInBasketRepository extends JpaRepository<AddInBasket, Integer> {
	...
	@Query(value = "SELECT dc.categoryName,d.dishName,d.dishPrize,d.dishDescription "
			+ "FROM Dish as d INNER JOIN DishCategory as dc ON d.categoryId = dc.id "  
			+ "WHERE d.dId IN "
	...
	@Query(value = "SELECT dc.id,d.dId "
			+ "FROM Dish as d INNER JOIN DishCategory as dc ON d.categoryId = dc.id "  
			+ "WHERE d.dId IN "
...
+ "INNER JOIN Dish d ON d.aB.IDdish = d.dId"	
+ "WHERE  exists "	
+ "(SELECT d.dId,dc.id,d.dishName ,dc.categoryName" 
+ "FROM d INNER JOIN dishCategory dc ON dc.d.categoryId = dc.id"
