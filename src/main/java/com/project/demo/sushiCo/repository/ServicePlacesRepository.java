...
public interface ServicePlacesRepository extends JpaRepository<ServicePlaces, Integer> {
	...
	@Modifying
	@Query(" Update ServicePlaces sp Set sp.service_Places And sp.shippingCost Where EXISTS "
			+ " ( Select sp.id , r.idRestorant From ServicePlaces sp INNER JOIN Restorant r ON r.sp.service_idR = r.idRestorant "
			+ " Where sp.id =: id And r.idRestorant =: idRestorant	)")
	ServicePlaces update(Integer idRestorant, Integer id, @Valid ServicePlacesDto placesDto,
			@Valid ServicePlacesDto sPlaces);
