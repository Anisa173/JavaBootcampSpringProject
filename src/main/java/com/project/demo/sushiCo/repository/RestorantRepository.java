...
public interface RestorantRepository extends JpaRepository<Restorant, Integer> {
	...
	@Query("Update Restorant  Set deleted = true Where r.idRestorant =: ?1 And aw.adminIdWeb =: ?2 "
			+ " From Restorant r " + " Where aw.id IN "
	...
	@Query("Update sR Set sR.rest =: rest From SelectWhichYouPreferForm sR Where idSelect =: idSelect ")
	SelectWhichYouPreferForm updateRprefered(@Valid SelectWhichYouPreferForm restorantSelected,
			@Valid SelectWhichYouPreferFormDto restorantSelected1, Integer idSelect);
	...
	@Query("Select r From Restorant r INNER JOIN User aw ON aw.r.adminIdWeb = aw.id "
			+ "	Where  aw.adminIdWeb =: adminIdWeb ")
	List<Restorant> getAllRestorants(Integer adminIdWeb);
